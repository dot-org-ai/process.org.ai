...
level: Activity
verb: 'Manage'
object: 'Flow Of Returned Products'
...
> Business-as-Code definition for manage flow of returned products. Models the inbound logistics processes to ensure efficient movement, storage, and compliance of goods throughout the supply chain.
...
```mermaid
graph TD
    A[Plan and manage inbound material flow] --> B[Manage flow of returned products]
...
| Action | Description |
|--------|-------------|
| manageActivity | Manage flow of returned products following established logistics procedures |
| assessRequirements | Evaluate operational requirements for manage flow of returned products |
...
| Event | Description |
|-------|-------------|
| requirementsAssessed | Operational requirements evaluated for manage flow of returned products |
...
| activityCompleted | Manage flow of returned products completed and documented |

## Searches

| Search | Description |
|--------|-------------|
| getOperationalStatus | Retrieve current status of manage flow of returned products activities |
...
```typescript
import { manageFlowOfReturnedProducts } from '@headlessly/manage-flow-of-returned-products'

const client = manageFlowOfReturnedProducts()
...
const requirements = await client.assessRequirements({
  ...
  scope: 'flow-of-returned-products',
  period: '2025-08'
})
