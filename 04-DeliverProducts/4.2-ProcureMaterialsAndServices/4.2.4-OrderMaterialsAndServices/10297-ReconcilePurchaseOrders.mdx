...
> Business-as-Code definition for reconcile purchase orders. Models the ordering processes within procurement to ensure materials and services are acquired efficiently and cost-effectively.
...
```mermaid
graph TD
    A[Order materials and services] --> B[Reconcile purchase orders]
...
| Action | Description |
|--------|-------------|
| reconcilePurchase | Reconcile purchase orders based on organizational requirements |
...
| executeAction | Carry out reconcile purchase orders tasks and document outcomes |
...
| actionExecuted | Reconcile purchase orders tasks completed and documented |
...
| Search | Description |
|--------|-------------|
| getProcurementStatus | Retrieve current status of reconcile purchase orders activities |
...
| getProcurementHistory | Query historical data for purchase orders activities |
...
|-----|-------------|------|
| Procurement Cycle Time | Average time to complete reconcile purchase orders | Days |
...
```typescript
import { reconcilePurchaseOrders } from '@headlessly/reconcile-purchase-orders'

const client = reconcilePurchaseOrders()
