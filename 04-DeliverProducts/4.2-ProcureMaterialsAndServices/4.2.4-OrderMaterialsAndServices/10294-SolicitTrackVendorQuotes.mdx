...
> Business-as-Code definition for solicit/track vendor quotes. Models the ordering processes within procurement to ensure materials and services are acquired efficiently and cost-effectively.
...
```mermaid
graph TD
    A[Order materials and services] --> B[Solicit/Track vendor quotes]
...
| Action | Description |
|--------|-------------|
| solicitVendorQuotes | Solicit vendor quotes based on organizational requirements |
...
| executeAction | Carry out solicit/track vendor quotes tasks and document outcomes |
...
| actionExecuted | Solicit/Track vendor quotes tasks completed and documented |
...
| Search | Description |
|--------|-------------|
| getProcurementStatus | Retrieve current status of solicit/track vendor quotes activities |
...
| getProcurementHistory | Query historical data for vendor quotes activities |
...
|-----|-------------|------|
| Procurement Cycle Time | Average time to complete solicit/track vendor quotes | Days |
...
```typescript
import { solicitTrackVendorQuotes } from '@headlessly/solicit-track-vendor-quotes'

const client = solicitTrackVendorQuotes()
