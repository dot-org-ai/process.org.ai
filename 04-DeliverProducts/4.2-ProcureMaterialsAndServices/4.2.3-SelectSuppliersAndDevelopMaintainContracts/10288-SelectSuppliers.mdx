...
> Business-as-Code definition for select suppliers. Models the supplier-management processes within procurement to ensure materials and services are acquired efficiently and cost-effectively.
...
```mermaid
graph TD
    A[Select suppliers and develop/maintain contracts] --> B[Select suppliers]
...
| Action | Description |
|--------|-------------|
| selectSuppliers | Select suppliers based on organizational requirements |
...
| executeAction | Carry out select suppliers tasks and document outcomes |
...
| actionExecuted | Select suppliers tasks completed and documented |
...
| Search | Description |
|--------|-------------|
| getProcurementStatus | Retrieve current status of select suppliers activities |
...
| getProcurementHistory | Query historical data for suppliers activities |
...
|-----|-------------|------|
| Procurement Cycle Time | Average time to complete select suppliers | Days |
...
```typescript
import { selectSuppliers } from '@headlessly/select-suppliers'

const client = selectSuppliers()
